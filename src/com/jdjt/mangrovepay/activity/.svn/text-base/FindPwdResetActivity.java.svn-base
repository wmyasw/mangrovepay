 ...
 * @date 2014-11-27
 */
@InjectLayer(value = R.layout.re_password, parent = R.id.center_common, isTitle = true)
public class FindPwdResetActivity extends BaseFragmentActivity implements
		ValidationListener {
	@Password(maxLength = 18, minLength = 6, message = "密码不符合规则，请重新输入", order = 1)
	...
	public void onValidationSucceeded() {
		...
		Ioc.getIoc().getLogger()
				.i("调用后台重置密码接口：[" + Url.METHOD_RESETPASSWORD + "]");
		String 	url = Url.METHOD_RESETPASSWORD;
	...
	public void result(ResponseEntity r) {
		...
		case FastHttp.result_ok:
			...
			if (ResultParse.isResultOK(r, this)) {
				...
				Ioc.getIoc().getLogger().i("重置密码成功");
					MangrovePayApp.app.setData("passwrod", new_pwd.getText()
							+ "");
	...
	public void init() {
		...
		super.textview_title.setText("重置密码");
	}
